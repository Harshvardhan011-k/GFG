...
// Piggy Bank - Add Money
router.post('/piggy-bank/add', (req, res) => {
    const { userId, amount } = req.body;
    db.run('UPDATE users SET piggy_balance = piggy_balance + ? WHERE id = ?', [amount, userId], function (err) {
        if (err) return res.status(500).json({ error: err.message });
        res.json({ message: 'Added to Piggy Bank', balance: this.changes });
    });
});

// Piggy Bank - Break (Withdraw to Main Balance)
router.post('/piggy-bank/break', (req, res) => {
    const { userId, destination = 'bonds' } = req.body;

    db.serialize(() => {
        db.get('SELECT piggy_balance FROM users WHERE id = ?', [userId], (err, row) => {
            if (err) return res.status(500).json({ error: err.message });
            if (!row || row.piggy_balance <= 0) return res.status(400).json({ error: 'Piggy bank is empty' });

            const amount = row.piggy_balance;
            ...
            // Reset piggy balance
            db.run('UPDATE users SET piggy_balance = 0 WHERE id = ?', [userId], (err) => {
                if (err) return res.status(500).json({ error: err.message });
            });

            // Record transaction with the chosen destination as asset_type
            db.run(
                'INSERT INTO transactions (user_id, amount, type, asset_type) VALUES (?, ?, ?, ?)',
                [userId, amount, 'credit', destination],
                function (err) {
                    if (err) return res.status(500).json({ error: err.message });
                    res.json({ message: 'Piggy bank broken!', amount, destination });
                }
            );
        });
    });
